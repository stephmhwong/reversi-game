...
void setBoard(char board[][26], int n) {
    for (int i = 0; i < n; i ++) // fill array with 'U' for [n][n] 
        for (int j = 0; j < n; j ++)
            board[i][j] = 'U';
 
    board[(n/2)-1][(n/2)-1] = 'W'; // fill in the center 4 pieces of array from above 'B' and 'W' for specific pieces
    board[(n/2)-1][n/2] = 'B';
    board[n/2][n/2] = 'W';
    board[n/2][(n/2)-1] = 'B';
}
...
void printBoard(char board[][26], int n){
    printf("  "); // printing the headers a-z in first row according to n --- NOTE: 'a' = 97 for %c
    for (int i = 0; i < n; i++)
        printf("%c", 97 + i);
    printf("\n");
...
bool checkLegalInDirection(char board[][26], int n, int row, int col, char colour, int deltaRow, int deltaCol) {
    ...
    if (colour == 'W') // WHITE PIECE: // Same logic as black piece but instead check for 'B'
        if (board[row][col] == 'U') // will return true if the specified space is 'U' and all pieces in that direction are 'B' until stopped by a 'W' piece
            for (int i = 1; (row + i * deltaRow) < n && (row + i * deltaRow) >= 0 && (col + i * deltaCol) < n && 
...
int countPieces(const char board[][26], int n, char colour){
    int count = 0; // initialize a counter to count the number of pieces, increase counter by 1 for every space occupied by specified colour
...
int countValidMoves(char board[][26], int n, char colour){
    int validMoves = 0; // initialize counter for validMoves
...
int makeMove(const char board[26][26], int n, char turn, int *row, int *col) {
    ...
    int score = 0, highestScore = 0; // initialize variable for score and highest score
    
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++){
            score = findScore(board, n, i, j, turn); // find score, checking validy of moves in included in findScore
            // if the score for the position is greater than the highest score, change the "best position" row and column to the current position
